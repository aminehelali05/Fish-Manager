...
function asciiTextClient($s){
    // replace euro symbol with DT label and transliterate to ASCII
...
$pdf->SetFont($font,'',12);
$pdf->Cell(0,6, asciiTextClient('Client: '.$client['nom'].' '.$client['prenom']),0,1);
$pdf->Cell(0,6, asciiTextClient('Telephone: '.$client['telephone']),0,1);
$pdf->Ln(6);
...
if($orders && mysqli_num_rows($orders) > 0){
    while($o = mysqli_fetch_assoc($orders)){
        ...
        while($it = mysqli_fetch_assoc($items)){
            ...
            $dr = $q / 10.0;
            $pdf->Cell(70,6, asciiTextClient($it['nom_fish']),1,0,'L');
            $pdf->Cell(30,6, number_format($q,2),1,0,'C');
            $pdf->Cell(30,6, number_format($dr,2),1,0,'C');
            $pdf->Cell(30,6, number_format($it['prix_vente'],2).' DT',1,0,'R');
            $pdf->Cell(40,6, number_format($it['total'],2).' DT',1,1,'R');
            $orderTotal += floatval($it['total']);
        }
        $pdf->SetFont('Arial','B',11);
        $pdf->Cell(150,7,'Sous-total commande',1,0,'R');
        $pdf->Cell(40,7,number_format($orderTotal,2).' DT',1,1,'R');
        $pdf->Ln(6);
    ...
    $pdf->Cell(40,9,number_format($overallTotal,2).' DT',1,1,'R');

} else {
    $pdf->Cell(0,6,'Aucune commande trouvee pour ce client.',0,1);
}
